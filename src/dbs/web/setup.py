...
def validate_netscape_cookies(text: str) -> None:
    """Raise :class:`ValueError` unless ``text`` looks like a Netscape cookies.txt.

    Accepts the conventional ``# Netscape HTTP Cookie File`` header, or at least
    one well-formed tab-delimited 7-field cookie row (blank lines and ``#``
    comments are skipped). Used to reject obviously-wrong uploads before they're
    written into place.
    """
    lines = [ln for ln in text.splitlines() if ln.strip() and not ln.startswith("#")]
    if any(len(ln.split("\t")) == 7 for ln in lines):
        return
    if "Netscape HTTP Cookie File" in text:
        return
    raise ValueError("doesn't look like a Netscape cookies.txt (expected tab-delimited rows)")


def validate_storage_state(text: str) -> dict:
    """Parse ``text`` as a Playwright ``storageState`` JSON, or raise :class:`ValueError`."""
    try:
        state = json.loads(text)
    except json.JSONDecodeError as exc:
        raise ValueError(f"not valid JSON: {exc}") from exc
    if not isinstance(state, dict) or "cookies" not in state or "origins" not in state:
        raise ValueError("doesn't look like a Playwright storageState (expected 'cookies' and 'origins' keys)")
    return state


def extract_session_zip(data: bytes, target_dir) -> None:
    """Extract a zipped Playwright profile directory into ``target_dir``.

    Guards against zip-slip: every entry must resolve to a path *inside*
    ``target_dir`` once extracted, or the whole upload is rejected before
    anything is written.
    """
    import io
    import zipfile
