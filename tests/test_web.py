...
# --- desktop-capture import (route-level) -----------------------------------


def test_import_disabled_403(client):
    r = client.post("/api/connectors/youtube/import", files={"file": ("c.txt", b"x")})
    assert r.status_code == 403


def test_import_no_auth_capture_400(setup_client):
    r = setup_client.post("/api/connectors/raindrop/import", files={"file": ("c.txt", b"x")})
    assert r.status_code == 400


def test_import_unknown_connector_404(setup_client):
    r = setup_client.post("/api/connectors/nope/import", files={"file": ("c.txt", b"x")})
    assert r.status_code == 404


def test_import_cookies_writes_target_and_sets_env(setup_client, tmp_path):
    cookies = "# Netscape HTTP Cookie File\n.youtube.com\tTRUE\t/\tTRUE\t0\tSID\tabc\n"
    r = setup_client.post(
        "/api/connectors/youtube/import", files={"file": ("cookies.txt", cookies.encode())}
    )
    assert r.status_code == 200
    body = r.json()
    assert body["secret_key"] == "YOUTUBE_COOKIES_FILE"
    assert Path(body["target"]).read_text(encoding="utf-8") == cookies
    assert "YOUTUBE_COOKIES_FILE=" in (tmp_path / ".env").read_text(encoding="utf-8")


def test_import_cookies_rejects_malformed_upload_without_writing(setup_client, tmp_path):
    r = setup_client.post(
        "/api/connectors/youtube/import", files={"file": ("cookies.txt", b"garbage")}
    )
    assert r.status_code == 400
    assert not (tmp_path / ".env").exists()


def test_import_session_zip_extracts_and_sets_env(setup_client):
    import io
    import zipfile

    buf = io.BytesIO()
    with zipfile.ZipFile(buf, "w") as zf:
        zf.writestr("Default/Preferences", b"{}")
    r = setup_client.post(
        "/api/connectors/skool/import", files={"file": ("session.zip", buf.getvalue())}
    )
    assert r.status_code == 200
    body = r.json()
    assert body["secret_key"] == "SKOOL_SESSION_DIR"
    assert (Path(body["target"]) / "Default" / "Preferences").exists()


def test_import_session_zip_rejects_traversal_without_writing(setup_client, tmp_path):
    import io
    import zipfile
...
def test_import_storage_state_writes_sibling_cookies(setup_client, monkeypatch):
    # No built-in connector currently uses browser_storage_state + a
    # target_dir_option (per-source) capture -- exercise the framework's
    # support for it via skool, whose config already has a directory option.
    from dbs.connectors.skool import SkoolConnector
    from dbs.core.capabilities import AuthCapture

    monkeypatch.setattr(
        SkoolConnector, "auth_capture",
        AuthCapture(
            kind="browser_storage_state", secret_key="SKOOL_SESSION_DIR",
            login_url="https://www.skool.com/login", label="Skool login",
            target_dir_option="downloads_dir", target_path=".auth/storage_state.json",
        ),
    )
    state = {"cookies": [{"name": "sid", "value": "v", "domain": "skool.com", "path": "/"}], "origins": []}
    r = setup_client.post(
        "/api/sources/courses/import",
        files={"file": ("storage_state.json", json.dumps(state).encode())},
    )
    assert r.status_code == 200
    target = Path(r.json()["target"])
    assert json.loads(target.read_text())["cookies"][0]["name"] == "sid"
    sibling = target.parent / "cookies.txt"
    assert "sid" in sibling.read_text()


def test_import_source_unknown_source_404(setup_client):
    r = setup_client.post("/api/sources/nope/import", files={"file": ("f", b"x")})
    assert r.status_code == 404
...
# --- desktop-capture import: validation + extraction (unit) ----------------


def test_validate_netscape_cookies_accepts_header_only():
    from dbs.web.setup import validate_netscape_cookies

    validate_netscape_cookies("# Netscape HTTP Cookie File\n# Generated by Daily Backup System\n")


def test_validate_netscape_cookies_accepts_tab_delimited_row():
    from dbs.web.setup import validate_netscape_cookies

    validate_netscape_cookies(".youtube.com\tTRUE\t/\tTRUE\t0\tSID\tabc\n")


def test_validate_netscape_cookies_rejects_garbage():
    from dbs.web.setup import validate_netscape_cookies

    with pytest.raises(ValueError, match="Netscape"):
        validate_netscape_cookies("not a cookies file at all")


def test_validate_storage_state_accepts_playwright_shape():
    from dbs.web.setup import validate_storage_state

    state = validate_storage_state(json.dumps({"cookies": [{"name": "a"}], "origins": []}))
    assert state["cookies"][0]["name"] == "a"


def test_validate_storage_state_rejects_bad_json():
    from dbs.web.setup import validate_storage_state
...
def test_validate_storage_state_rejects_missing_keys():
    from dbs.web.setup import validate_storage_state

    with pytest.raises(ValueError, match="storageState"):
        validate_storage_state(json.dumps({"foo": "bar"}))


def test_extract_session_zip_writes_files(tmp_path):
    import io
    import zipfile

    from dbs.web.setup import extract_session_zip

    buf = io.BytesIO()
    with zipfile.ZipFile(buf, "w") as zf:
        zf.writestr("Default/Cookies", b"sqlite-ish bytes")
        zf.writestr("Default/Preferences", b"{}")
    target = tmp_path / "profile"
    extract_session_zip(buf.getvalue(), target)
    assert (target / "Default" / "Cookies").read_bytes() == b"sqlite-ish bytes"


def test_extract_session_zip_rejects_zip_slip(tmp_path):
    import io
    import zipfile
...
def test_extract_session_zip_rejects_bad_zip(tmp_path):
    from dbs.web.setup import extract_session_zip

    with pytest.raises(ValueError, match="not a valid zip"):
        extract_session_zip(b"not a zip", tmp_path / "profile")
